...
namespace OSD
{
    ...
    class Panels
    {
        ...
        public uint8_t rssimin = 0;
        public uint8_t rssimax = 255;
        ...
        osd.openPanel();
        {
        osd.printf("%c%3.0f%c", 0x12, eff, 0x82);
        }
        osd.closePanel();
         return 0;
        }
        ...
        public int panWindSpeed(int first_col, int first_line)
        {
            ...
            osd.openPanel();
            {
                osd.printf("%c%3.0f%c|%c%c%2.0f%c", 0xd4, (double)(osd_windspeed * 3.6), 0x81, 0xA4, 0xA5, (double)(osd_windspeedz * 3.6), 0x81);
            }
            osd.closePanel();
            return 0;
        }
        ...
        public int panCur_A(int first_col, int first_line)
        {
            ...
            osd.openPanel();
            {
                osd.printf("%c%5.2f%c%c", 0xcc, (osd_curr_A * .01), 0x8F);
            }
            osd.closePanel();
            return 0;
        }
        ...
        public int panAlt(int first_col, int first_line)
        {
            osd.setPanel(first_col, first_line);
            osd.openPanel();
            //osd.printf("%c%5.0f%c",0xce, (double)(osd_alt - osd_home_alt), 0x8D);
            osd.printf("%c%5.0f%c", 0xcd, (double)(osd_alt), 0x8D);
            osd.closePanel();
            return 0;
        }
        /* **************************************************************** */
        ...
        public int panVel(int first_col, int first_line)
        {
            osd.setPanel(first_col, first_line);
            osd.openPanel();
            osd.printf("%c%3.0f%c", 0xd3, (double)osd_groundspeed, 0x81);
            osd.closePanel();
            return 0;
        }
        ...
        public int panAirSpeed(int first_col, int first_line)
        {
            osd.setPanel(first_col, first_line);
            osd.openPanel();
            osd.printf("%c%3.0f%c", 0xd2, (double)(osd_airspeed), 0x81);
            osd.closePanel();
            return 0;
        }
        ...
        public int panBatteryPercent(int first_col, int first_line)
        {
            osd.setPanel(first_col, first_line);
            osd.openPanel();
            osd.printf("%c%3.0i%c", 0xB4, (osd_battery_remaining), 0x25);
            osd.closePanel();
            return 0;
        }
        ...
        public int panHomeDis(int first_col, int first_line)
        {
            osd.setPanel(first_col, first_line);
            osd.openPanel();
            osd.printf("%c%5.0f%c", 0x11, (double)osd_home_distance, 0x8D);
            osd.closePanel();
            return 0;
        }
        ...
        public int panBatt_A(int first_col, int first_line)
        {
            osd.setPanel(first_col, first_line);
            osd.openPanel();
            //osd.printf("%c%5.2f%c%c", 0xcb,(double)osd_vbat, 0x8E, osd_battery_pic);
            osd.printf("%c%5.2f%c", 0xcb, (double)osd_vbat, 0x8E);
            osd.closePanel();
            return 0;
        }
        ...
        public int panGPL(int first_col, int first_line)
        {
            ...
            switch (osd_fix_type)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    osd.printf_P(PSTR("\x01"));//If not APM, x01 would show 2D fix
                    break;
                case 3:
                    osd.printf_P(PSTR("\x02"));//If not APM, x02 would show 3D fix
                    break;
            }
        ...
        public int panRose(int first_col, int first_line)
        {
            ...
            osd.printf("%s|%c%s%c", "\x20\xc0\xc0\xc0\xc0\xc0\xc7\xc0\xc0\xc0\xc0\xc0\x20", 0xd0, ecp1252.GetString(buf_show), 0xd1);
            osd.closePanel();
            return 0;
        }
        ...
        public int panFlightMode(int first_col, int first_line)
        {
            ...
            if (apm_mav_type == 2)//ArduCopter
            {
                ...
                if (osd_mode == 100) osd.printf_P(PSTR("\x1A \x1b\x1c"));//Stabilize
                if (osd_mode == 101) osd.printf_P(PSTR("\x1A \x1d\x1e"));//Acrobatic
                if (osd_mode == 102) osd.printf_P(PSTR("\x1A \x1f\x7f"));//Alt Hold
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_AUTO && osd_nav_mode == (byte)ArdupilotMega.MAVLink.MAV_NAV.MAV_NAV_WAYPOINT) osd.printf_P(PSTR("\x1A \x1b\x80"));//Auto
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_GUIDED && osd_nav_mode == (byte)ArdupilotMega.MAVLink.MAV_NAV.MAV_NAV_WAYPOINT) osd.printf_P(PSTR("\x1A \x83\x84"));//Guided
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_AUTO && osd_nav_mode == (byte)ArdupilotMega.MAVLink.MAV_NAV.MAV_NAV_HOLD) osd.printf_P(PSTR("\x1A \x85\x86"));//Loiter
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_AUTO && osd_nav_mode == (byte)ArdupilotMega.MAVLink.MAV_NAV.MAV_NAV_RETURNING) osd.printf_P(PSTR("\x1A \x87\x89"));//Return to Launch
                if (osd_mode == 107) osd.printf_P(PSTR("\x1A \x8a\x8b")); // Circle
                if (osd_mode == 108) osd.printf_P(PSTR("\x1A \x1f\x8c")); // Position
                if (osd_mode == 109) osd.printf_P(PSTR("\x1A \xb5\xb6")); // Land
                if (osd_mode == 110) osd.printf_P(PSTR("\x1A \xb7\xb8")); // OF_Loiter
            }
            else if (apm_mav_type == 1) // arduplane
            {
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_TEST1 && osd_nav_mode == (byte)ArdupilotMega.MAVLink.MAV_NAV.MAV_NAV_VECTOR) osd.printf_P(PSTR("\x1A \x1b\x1c"));//Stabilize
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_MANUAL && osd_nav_mode == (byte)ArdupilotMega.MAVLink.MAV_NAV.MAV_NAV_VECTOR) osd.printf_P(PSTR("\x1A \xe4\x80"));//Manual
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_AUTO && osd_nav_mode == (byte)ArdupilotMega.MAVLink.MAV_NAV.MAV_NAV_LOITER) osd.printf_P(PSTR("\x1A \x85\x86"));//Loiter
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_AUTO && osd_nav_mode == (byte)ArdupilotMega.MAVLink.MAV_NAV.MAV_NAV_RETURNING) osd.printf_P(PSTR("\x1A \x87\x89"));//Return to Launch
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_TEST2 && osd_nav_mode == 1) osd.printf_P(PSTR("\x1A fbwa"));//FLY_BY_WIRE_A
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_TEST2 && osd_nav_mode == 2) osd.printf_P(PSTR("\x1A fbwb"));//FLY_BY_WIRE_B
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_GUIDED) osd.printf_P(PSTR("\x1A \x83\x84"));//GUIDED
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_AUTO && osd_nav_mode == (byte)ArdupilotMega.MAVLink.MAV_NAV.MAV_NAV_WAYPOINT) osd.printf_P(PSTR("\x1A \x1b\x80"));//AUTO
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_AUTO && osd_nav_mode == (byte)ArdupilotMega.MAVLink.MAV_NAV.MAV_NAV_RETURNING) osd.printf_P(PSTR("\x1A \x87\x89"));//RTL
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_AUTO && osd_nav_mode == (byte)ArdupilotMega.MAVLink.MAV_NAV.MAV_NAV_LOITER) osd.printf_P(PSTR("\x1A \x85\x86"));//LOITER
                if (osd_mode == (byte)ArdupilotMega.MAVLink.MAV_MODE.MAV_MODE_TEST3) osd.printf_P(PSTR("\x1A \x8a\x8b"));//CIRCLE
            }
